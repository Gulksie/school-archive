...
# the code starts at line 71 lol
...
# ## Background
# 
# Your task is to write a program for a system that will determine the speed at which to fire a projectile at enemy zombies. Lucky for us, our system is equipped with a proximity sensor that displays the distance from our projectile launcher to the nearest zombie. The use case for the system is described below:
# 
# 1.   Provide the system the distance to the nearest zombie.
# 2.   The system displays the angle and velocity required to set the projectile launcher in order to hit the zombie.
# 
...
# 
# Fortunately for us, these zombies are extremely slow. Thus, we can treat this scenario as a simple projectile motion problem. After some clever manipulation of the kinematic equations, we can derive a formula used to calculate the velocity **V** at which to fire our projectile in order to travel distance **d** in the horizontal direction at a given angle **theta** (in radians). This equation is given below:
# 
# <br/><br/>
# \begin{align}
#   V = \sqrt{\frac{g*d}{sin(2*theta)}} \tag{1}
# \end{align}
# <br/><br/>
# 
# The constant **g** is the acceleration due to gravity on earth (9.81m/s). For simplicity, we assume that all units are SI units.
# 
# 
# 
...
#     * g = 9.81
# 2. The program randomly determines the angle theta. The angle must conform to the following requirement: *0 < theta ≤ 45*.<br/><br/>*(Hint: you can use the random.random() function from the random library to generate random numbers between 0 and 1. If you multiply this result by 45, you will have a random number between 1 and 45)*
# 
# 3. The angle from the previous step is converted to radians, and stored in the variable **theta_rads**.
# 
# 4. The program prompts the user to enter **d** (the distance to the zombie) accompanied by the following message: 
# > *“Please enter the distance to our target zombie: ”*
# 
#   The value entered by the user is stored in the variable **d**. (Note that this value needs to be type float)
# 
# 5. The program calculates **V** with equation (1) using the values for **g**, **d**, and **theta_rads**.
# 
# 6. Once **V** is calculated, the program prints the following messages on seperate lines: 
# > *“Ready for launch!”<br/>
# > “Set angle to &lt;angle value&gt; degrees”<br/>
# > “Set speed to &lt;velocity&gt; m/s“*
# 
#   Where *&lt;angle&gt;* and *&lt;velocity&gt;* are the actual values for **theta** and **V**, respectively. An example output is shown below:
# 
#   >*“Ready for launch!”<br/>
#   >“Set angle to 42.2325353123”<br/>
#   >“Set speed to 400.00”<br/>*
...
# YOUR CODE HERE
from random import random as rndfloat
import math

g = 9.81  # for use later when calculating v

# error checking to make sure user input is a float and within range
while 1:
    d = input("Please enter the distance to our target zombie: ")  # user input

    # will throw ValueError if d is not convertable to a float
    try:
        d= float(d)

        if d <= 0:  # checks if d is in the range of 0 < d < infinity 
            print("Distance must be greater than 0, please try again.\n")
            continue

        break
    except ValueError:  # d is unconvertable to a float (probably has text or punctuation chars in it)
        print("Unable to convert input to a number. Please try again\n")

theta = rndfloat() * 45  # pick a random number between 1 and 45 degrees
theta_rads = math.radians(theta)  # convert above function to radians to use in calculation

v = math.sqrt(g*d/math.sin(theta_rads*2))  # calculate velocity using given formula

# output
# uses a fstring and \n for newlines because it is quicker to write and only takes one line
...
# 
# 2. Would the behaviour of your program change if the line g = 9.81 was defined *after* you calculated the value for V? Justify your answer.
# 
# Please answer all questions in the cell below!

# ```
# 
# 1. My program has error checking to deal with non-float inputs and inputs <= 0. However, if I didn't add that, a negative d value would make the math.sqrt() function throw a ValueError (can't take the square root of a negative value). A non-numeric input would also throw a value error, but instead at the float() function rather than in the sqrt() function.
# 
# 2. Yes it would. This is because the calculation will not be able to proceed if the g variable is not properly defined, in which case it would throw a NameError. However, for some reason this error does not occur in the Jupyter environment (it occurred when I tested it in a standard python deployment).
# 
# ```
